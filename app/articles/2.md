...
The first thing to notice when writing tests with RSpec is it's structure of `it`, `expect`, `subject`, `describe` and
`context`.
...
Sometimes we will see `RSpec.describe` on the beginning of the test instead of `describe`. Both work work the same.
I prefer using just `describe` because is more clean.

## "Let" me tell you something

We use `let` do declare "variables" for our tests. The reason to use `let` instead of regular ruby variables is that
`let` will cache the result of it's call in the same `it`, but not in different `it`s.
...
On the example below, we want or method to "do something" when the redis key is true, and "do nothing" when the redis
...
Keep in mind that when we don't specify the variable when calling these hooks, it will use `:each` by default. There are
